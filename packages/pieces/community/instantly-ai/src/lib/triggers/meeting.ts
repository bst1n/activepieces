...
export const meetingTrigger = instantlyTriggerFactory.createGroupedWebhookTrigger({
  ...
  aiDescription:
    'Fires when a meeting is booked or completed with an Instantly lead, with the selected meeting events chosen per flow and a single webhook registered automatically. Delivers the raw event payload including campaign, lead email, and unibox link. Optionally scoped to one campaign.',
  eventOptions: [
