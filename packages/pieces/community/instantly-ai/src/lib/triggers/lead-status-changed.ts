...
export const leadStatusChangedTrigger = instantlyTriggerFactory.createGroupedWebhookTrigger({
  ...
  aiDescription:
    'Fires when the status of an Instantly lead changes — interested, not interested, neutral, unsubscribed, closed, out of office, or wrong person — with the selected statuses chosen per flow and a single webhook registered automatically. Delivers the raw event payload including campaign, lead email, and unibox link. Optionally scoped to one campaign.',
  eventOptions: [
