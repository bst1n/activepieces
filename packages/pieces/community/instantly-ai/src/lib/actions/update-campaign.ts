...
export const updateCampaignAction = createAction({
  ...
  aiMetadata: { description: 'Updates an existing Instantly campaign by campaign ID, changing only the delivery settings supplied (name, daily limit, tracking, stop-on-reply, stop-on-auto-reply, daily max leads, text-only); fields left empty are not sent and keep their current values. Use to tune a live campaign without recreating it. Idempotent — applying the same values repeatedly leaves the campaign in the same state.', idempotent: true },
  props: {
